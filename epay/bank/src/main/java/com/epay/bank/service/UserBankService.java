    ...
    public PaystackAccountData verifyExternal(String accountNumber, String bankCode) {
        try {
            ...
            if (result == null || !result.isStatus() || result.getData() == null) {
                ...
                log.error(
                        "[Bank] verifyExternal failed: account={} bankCode={} reason={}",
                        accountNumber,
                        bankCode,
                        msg
                );
            ...
            PaystackAccountData data = result.getData();
            data.setBankCode(bankCode);

            log.info(
                    "[Bank] Verified account={} bankCode={} name={}",
                    accountNumber,
                    bankCode,
                    data.getAccountName()
            );
            ...
            log.error(
                    "[Bank] Paystack HTTP error verifying account: {} {}",
                    e.getStatusCode(),
                    e.getMessage()
            );
