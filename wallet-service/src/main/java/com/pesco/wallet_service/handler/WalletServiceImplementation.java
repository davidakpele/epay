...
public class WalletServiceImplementation extends WalletServiceGrpc.WalletServiceImplBase {
    ...
    @Override
    public void findUserWalletPin(FindUserWalletPinRequest request, 
            StreamObserver<FindUserWalletPinResponse> responseObserver) {
        try {
            Optional<Wallet> walletOpt = walletRepository.findById(request.getWalletId());

            if (walletOpt.isEmpty()) {
                responseObserver.onNext(FindUserWalletPinResponse.newBuilder()
            ...
            WalletSettings settings = settingsOpt.get();
            responseObserver.onNext(FindUserWalletPinResponse.newBuilder()
                    .setWalletId(settings.getWallet().getId())
                    .setPassword(settings.getPassword())
                    .setIsSecure(settings.isIsSecure())
                    .build());
            responseObserver.onCompleted();

        } catch (Exception e) {
            responseObserver.onError(Status.INTERNAL
                    .withDescription("Error retrieving wallet pin: " + e.getMessage())
                    .asRuntimeException());
        }
    }
