...
public class WalletController {
    ...
    public ResponseEntity<?> userSetTransferPin(@RequestBody CreateTransferPinRequest request,
        ...
        if (!request.getUsername().equals(requestUsername)) {
            ...
            error.put("message", "Fraudulent action detected. You are not the authorized user to operate this wallet.");
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(error);
        }
        ...
        if (wallet.isPresent()) {
            ...
            update.setPassword(passwordEncoder.encode(providedPin));
            update.setIsSecure(true);
        ...
        return ResponseEntity.badRequest().body("Sorry, system cannot update your wallet right now.");
    }
