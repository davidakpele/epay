...
const DepositModal = ({ isOpen, onClose, theme, onDepositSuccess }: DepositModalProps) => {
  ...
  const handleDeposit = async () => {
    ...
    try {
      ...
      const currency = extract_symbol?.currency || fiatCurrency;

      // Generate a unique idempotency key for this request
      const idempotencyKey = `deposit-${user_id}-${Date.now()}-${Math.random().toString(36).slice(2, 9)}`;
      ...
      let depositSystem = 'PAYSTACK';
      if (step === 'card') depositSystem = 'CARD';
      else if (step === 'ussd') depositSystem = 'USSD';
