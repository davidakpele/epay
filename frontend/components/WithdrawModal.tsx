...
const WithdrawModal = ({
  ...
  const fetchBankList = async () => {
    ...
    try {
      ...
      if (response && response.data.length > 0) {
        const options = response.data.map((bank: any) => ({
          value: bank.code,
          label: bank.name,
        }));
  ...
  const verifyAccountNumber = async (
    ...
    try {
      const response = await bankCollectionService.getUserBanks(
        accountNumber,
        bankCode,
      );
      ...
      if (response && response.success === true) {
        ...
        setErrors((prev) => ({
          ...prev,
          accountNumber: response?.message || "Account verification failed",
        }));
        setAccountName("");
      }
    } catch (error) {
      setErrors((prev) => ({
        ...prev,
        accountNumber: "Failed to verify account",
      }));
      setAccountName("");
    } finally {
  ...
  const saveBeneficiaryUser = async () => {
    ...
    try {
      ...
      const response =
        await beneficiaryService.saveBeneficiary(beneficiaryData);
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      if (walletData && Array.isArray(walletData)) {
        const activeCurrency = getFiat();
        const formattedWallets: WalletType[] = walletData.map(
          (wallet, index) => ({
            id: index + 1,
        ...
        const activeWallet = activeCurrency
          ? formattedWallets.find((w) => w.currency === activeCurrency)
          : null;
        const defaultWallet =
          activeWallet || formattedWallets.find((w) => w.isDefault);
  ...
  const validateForm = (): boolean => {
    ...
    if (step === "bank") {
      if (!selectedBankOption || !bankName)
    ...
    try {
      ...
      if (response) {
        if (
          ...
          if (token && userId) {
            const walletResponse = await walletService.getByUserId(
              userId,
              token,
            );
            setWalletContainer(
              walletResponse.wallet_balances,
              walletResponse.hasTransferPin,
          ...
          if (typeof window !== "undefined") {
            ...
            if ((window as any).refreshTransactionHistory)
              (window as any).refreshTransactionHistory();
          }
  ...
  const availableWallets = wallets.filter(
    (wallet) => !wallet.name.includes("Savings"),
  );
          ...
          <div className="drawer-body">
            {step === "selection" ? (
              <div className="options-list">
                <div
                  className="withdraw-option-card"
              ...
              <>
                <div className="wallet-selector">
                      ...
                      <div className="wallet-info">
                        ...
                        <span className="wallet-balance">
                          {formatBalance(
                            ...
                            getWallet(selectedWallet.currency)?.symbol ||
                  ...
                  {errors.wallet && (
                    <span className="error-message">{errors.wallet}</span>
                  )}
                </div>
                ...
                <div className="amount-section">
                  ...
                  {selectedWallet?.currency === "NGN" && (
                    <div className="quick-amounts">
                      {["50,000", "100,000", "200,000", "500,000"].map(
                        (val) => (
                          <button
                            key={val}
                    ...
                    <span className="summary-value total-value">
                      {selectedWallet
                        ? getWallet(selectedWallet.currency)?.symbol ||
                          selectedWallet.currency
                        : "₦"}
                      {formatNumberWithCommas(calculateTotal().toFixed(2))}
                    </span>
                  </div>
                </div>
              </>
                  ...
                  >
                    ...
                    <div className="wallet-details">
                      ...
                      <p>
                        {formatBalance(
                          wallet.balance,
                          getWallet(wallet.currency)?.symbol || "",
                        )}
                      </p>
                    </div>
      ...
      {showBeneficiaryModal && (
        <>
          <div
            className="status-modal-overlay"
            onClick={() =>
              !isSavingBeneficiary && setShowBeneficiaryModal(false)
            }
          />
          <div className={`status-modal beneficiary-modal ${theme}`}>
            <div className="status-modal-content">
              ...
              <div
                className="status-modal-actions"
