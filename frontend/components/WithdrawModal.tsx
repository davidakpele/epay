...
import { bankCollectionService, beneficiaryService, getActiveWallet, getFiat, getToken, getUserId, getUsername, getUserWalletId, getWallet, getWalletList, setFiat, setWalletContainer, updateNotificationContainer, uuidv4, walletService, withdrawService } from '@/app/api';
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      if (walletData && Array.isArray(walletData)) {
        // Use the currency the user last selected (stored in active_wallet / fiat)
        const activeCurrency = getActiveWallet() || getFiat();
        ...
        const formattedWallets: WalletType[] = walletData.map((wallet, index) => ({
          ...
          isDefault: activeCurrency
            ? wallet.currency_code === activeCurrency
            : index === 0
        }));
        setWallets(formattedWallets);

        // Pick the active wallet; fall back to first if nothing matched
        const activeWallet =
          formattedWallets.find(w => w.isDefault) ?? formattedWallets[0] ?? null;
        if (activeWallet) {
          setSelectedWallet(activeWallet);
          setFiat(activeWallet.currency);
        }
      }
