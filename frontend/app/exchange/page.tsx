...
const SwapPage = () => {
  ...
  const getRawRate = (from: string, to: string) =>
    mockExchangeRates[from]?.[to] || null;
  ...
  const fetchUserWallets = useCallback(async () => {
    try {
      ...
      if (!list) throw new Error("No wallet data");
      setUserWallets(list);
    } catch {
      setUserWallets([]);
  ...
  const getWalletBalance = (code: string) => {
    ...
    return w ? parseFloat(String(w.balance).replace(/,/g, "")) : 0;
  };

  const getWalletId = (code: string): string | null => {
    const w = userWallets.find((x: any) => x.currency_code === code);
    return w ? w.walletId || w.wallet_id || w.id || null : null;
  };
  ...
  const handleExchange = async () => {
    ...
    const targetWallet = getWalletId(toCurrency.code) || toCurrency.code;
    ...
    try {
      const response = await exchangeService.swap(payload);

      // xhrClient resolves on 2xx — also check body for a logical failure
      if (response?.success === false) {
        const errMsg = extractErrorMessage(response);
      ...
      updateNotificationContainer({
        type: "PAYMENTS",
        description: "Currency swap completed successfully",
      });
      showToast(
        `Swap complete! You received ${toCurrency.symbol}${toAmount}`,
        "success",
      );
      setFromAmount("");
      setToAmount("");
  ...
  /** Extract the message from an xhrClient error object and display it as-is. */
          ...
          <div className="main-container">
              ...
              <div className="swap-form-col">
                ...
                <div className="swap-wallet-balances">
                  <div className="swap-wallet-item">
                    <span className="swap-wallet-label">
                      {fromCurrency.code} Balance
                    </span>
                    <span className="swap-wallet-value">
                      {fromCurrency.symbol}
                      {getWalletBalance(fromCurrency.code).toLocaleString(
                        "en-US",
                  ...
                  <div className="swap-wallet-item">
                    ...
                    <span className="swap-wallet-value">
                      {toCurrency.symbol}
                      {getWalletBalance(toCurrency.code).toLocaleString(
                        "en-US",
                        { minimumFractionDigits: 2, maximumFractionDigits: 2 },
                      )}
                    </span>
                ...
                {fromAmount &&
                  parseFloat(fromAmount.replace(/,/g, "")) >
                    getWalletBalance(fromCurrency.code) && (
                    <span className="swap-field-error">
                      <AlertCircle size={12} /> Insufficient {fromCurrency.code}{" "}
                      balance
                    </span>
                  )}
                ...
                <div className="swap-field-group">
                  ...
                  >
                    ...
                    <span className="swap-currency-name">
                      {fromCurrency.name}
                    </span>
                ...
                <div className="swap-field-group">
                  ...
                  {fromCurrency.code === toCurrency.code && (
                    <span className="swap-field-error">
                      <AlertCircle size={12} /> From and To currencies must be
                      different
                    </span>
                  )}
                </div>
                ...
                {/* Transaction PIN */}
                <div className="swap-pin-section">
                  <div className="swap-pin-header">
                    <span
                      className="swap-section-label"
                    ...
                    {filteredFrom.length === 0 ? (
                      ...
                      filteredFrom.map((c) => (
                        ...
                        >
                          ...
                          <div
                            className={`radio-outer ${fromCurrency.code === c.code ? "checked" : ""}`}
                          >
                    ...
                    {filteredTo.length === 0 ? (
                      ...
                      filteredTo.map((c) => (
                        ...
                        >
                          ...
                          <div
                            className={`radio-outer ${toCurrency.code === c.code ? "checked" : ""}`}
                          >
                            <div className="radio-inner" />
                          </div>
                        </div>
            ...
            {showSuccessModal && exchangeSnapshot && (
              <>
                ...
                />
                <div
                  className={`status-modal success-modal ${theme === "dark" ? "dark" : ""}`}
                >
                  <div className="status-modal-content">
                    ...
                    <p className="status-modal-message">
                      Your currency swap has been completed.
                    </p>
                    <div className="status-modal-details">
                      <div className="status-detail-row">
                        <span className="status-detail-label">From</span>
                        <span className="status-detail-value">
                          {exchangeSnapshot.fromCurrency.symbol}
                          {exchangeSnapshot.fromAmount}{" "}
                          {exchangeSnapshot.fromCurrency.code}
                        </span>
                      </div>
                      <div className="status-detail-row">
                        <span className="status-detail-label">Rate</span>
                        <span className="status-detail-value">
                          1 {exchangeSnapshot.fromCurrency.code} ={" "}
                          {exchangeSnapshot.exchangeRate.toFixed(6)}{" "}
                          {exchangeSnapshot.toCurrency.code}
                        </span>
                      </div>
                      <div className="status-detail-row swap-summary-fee">
                        <span className="status-detail-label">Fee (1.5%)</span>
                        <span className="status-detail-value">
                          −{exchangeSnapshot.toCurrency.symbol}
                          {exchangeSnapshot.feeAmount}
                        </span>
                      </div>
                      <div className="status-detail-row">
                        ...
                        <span
                          ...
                          style={{ color: "var(--bg-main)", fontWeight: 700 }}
                        >
                          {exchangeSnapshot.toCurrency.symbol}
                          {exchangeSnapshot.toAmount}{" "}
                          {exchangeSnapshot.toCurrency.code}
                        </span>
                      </div>
                    </div>
                    <button
                      className="status-modal-btn success-btn"
                      onClick={() => setShowSuccessModal(false)}
                    >
                      Done
                    </button>
            ...
            {showFailModal && (
              <>
                ...
                />
                <div
                  className={`status-modal fail-modal ${theme === "dark" ? "dark" : ""}`}
                >
                  <div className="status-modal-content">
                    ...
                    <div className="status-modal-actions">
                      ...
                      <button
                        className="status-modal-btn fail-btn"
                        onClick={() => {
