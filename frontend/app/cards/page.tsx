...
const mapApiCard = (raw: any, userFullName: string): VirtualCard => {
  ...
  const network: CardNetwork =
    String(raw.card_type || raw.cardType || raw.network || "Visa").includes(
      "MASTER",
    ) ||
    String(raw.card_type || raw.cardType || raw.network || "").toLowerCase() ===
      "master"
      ? "Master"
      : "Visa";
...
const mapApiRequest = (raw: any): CardRequest => {
  ...
  return {
    ...
    network,
    amount: parseFloat(
      raw.initial_balance || raw.initialBalance || raw.amount || 0,
    ),
    status,
    requestedOn: fmt(raw.created_at || raw.createdAt || raw.requestedOn || ""),
    approvedOn:
      status === "Active"
        ? fmt(raw.updated_at || raw.updatedAt || raw.approvedOn || "")
        : undefined,
    flag: FLAG_MAP[currency] || FLAG_MAP["USD"],
  };
};
...
const VirtualCardsPage = () => {
  ...
  const showToast = (msg: string, type: "warning" | "success" = "warning") => {
    setToasts((prev) => {
      ...
      setTimeout(() => {
        setToasts((cur) =>
          cur.map((t) => (t.id === id ? { ...t, exiting: true } : t)),
        );
        setTimeout(
  ...
  const getCardGradient = (card: VirtualCard) => {
    if (card.status === "Pending" || card.status === "On Review")
      return "card-gradient-dark";
  ...
  const loadWalletBalance = useCallback(async () => {
    try {
      ...
      if (response?.wallet_balances) {
        const apiCurrencies: Currency[] = response.wallet_balances.map(
          (b: any) => ({
        ...
        setWalletContainer(
          response.wallet_balances,
          response.hasTransferPin,
          response.walletId,
        );
      }
    } catch (e) {
  ...
  const loadCards = useCallback(async () => {
    try {
      ...
      const cardsResponse =
        await virtualCardService.fetchUserVirtualCardsByUserId(userId);
  ...
  useEffect(() => {
    if (wallet?.wallet_balances) {
      const filtered = wallet.wallet_balances
        .filter(
          (w: any) => w.currency_code === "USD" || w.currency_code === "NGN",
        )
        .map((w: any) => ({
  ...
  const handleFundCard = async () => {
    ...
    setCards((prev) =>
      prev.map((c) =>
        c.id === fundModal.card?.id ? { ...c, balance: c.balance + amt } : c,
      ),
    );
    ...
    setIsSubmitting(false);
    showToast(
      `Card funded with ${getCurrencySymbol(fundModal.card!.currency)}${amt.toLocaleString()}!`,
      "success",
    );
  };
  ...
  const handleWithdraw = async () => {
    ...
    if (amt > (withdrawModal.card?.balance || 0)) {
      showToast("Insufficient card balance", "warning");
      return;
    }
    setIsSubmitting(true);
    // try {
    ...
    setIsSubmitting(false);
    showToast(
      `${getCurrencySymbol(withdrawModal.card!.currency)}${amt.toLocaleString()} withdrawn successfully!`,
      "success",
    );
  };
        ...
        <div className="scrollable-content">
          ...
          <div className="main-container">
            ...
            <div className="vc-topbar">
              <div>
                <h1 className="vc-title">
                  <span className="title-green">Virtual</span> Cards
                </h1>
                <p className="vc-subtitle">
                  Request and manage your virtual cards (USD, EUR, GBP)
                </p>
              </div>
            </div>
            ...
            <div className="vc-top-row">
              ...
              <div className="vc-wallets-card">
                <div className="vc-wallets-header">
                  <span className="vc-wallets-title">
                    Available Wallet Balance
                  </span>
                  <button
                    className="vc-eye-btn"
                ...
                <div className="vc-wallets-row">
                  {wallets.map((w, i) => (
                    <React.Fragment key={w.code}>
                      ...
                      {i < wallets.length - 1 && (
                        <div className="vc-wallet-divider" />
                      )}
                    </React.Fragment>
                  ))}
              ...
              <div className="vc-status-card">
                ...
                <div className="vc-status-row">
                  <div className="vc-status-item status-pending-bg">
                    <span className="vc-status-count">
                  ...
                  <div className="vc-request-meta">
                    <div className="vc-meta-chip">
                      ...
                      <div className="vc-meta-chip-body">
                        ...
                        <span className="vc-meta-chip-value">
                          {userFullName}
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="vc-meta-chip">
                      ...
                      <div className="vc-meta-chip-body">
                        <span className="vc-meta-chip-value fee-green">
                          {cardFees[cardNetwork]}
                        </span>
                      </div>
                    </div>
                  </div>
                  ...
                  {isCardsLoading ? (
                    ...
                    <div className="vc-empty-state">
                      ...
                      <p className="vc-empty-sub">
                        Your card request history will appear here once you
                        submit a request.
                      </p>
                    </div>
                  ) : (
                      ...
                      <div key={r.id} className="vc-history-item">
                        <div className="vc-history-top">
                          <div className="vc-history-top-left">
                            ...
                            />
                            <p className="vc-history-name">
                              {r.currency} Virtual Card
                            </p>
                          </div>
                          <span
                            className={`vc-history-status ${getStatusColor(r.status)}`}
                          >
                        ...
                        <div className="vc-history-meta">
                          ...
                          {r.approvedOn ? (
                            <>
                              <span className="vc-history-meta-approved">
                                Approved on:
                              </span>
                              <span>{r.approvedOn}</span>
                            </>
                          ) : (
                            <>
                              <span>Requested on:</span>
                              <span>{r.requestedOn}</span>
                            </>
                          )}
                        </div>
                      </div>
            ...
            <div className="vc-cards-section">
              <div className="vc-cards-header">
                <h3 className="vc-cards-title">
                  My <span className="title-green">Virtual Cards</span>
                </h3>
                <div className="vc-tabs">
                  {(
                    ["All Cards", "Active", "Pending", "On Review"] as const
                  ).map((tab) => (
                    <button
                      key={tab}
              ...
              {isCardsLoading ? (
                ...
                <div className="vc-cards-empty">
                  <div className="vc-empty-icon">
                    <CreditCardIcon size={24} color="#16a34a" />
                  </div>
                  ...
                  {filteredCards.map((card) => {
                    ...
                    return (
                      <div key={card.id} className="vc-card-wrapper">
                        {/* Card face */}
                        <div
                          className={`vc-card ${getCardGradient(card)} ${isLocked ? "vc-card-locked" : ""}`}
                        >
                          <div className="vc-card-row1">
                            <div className="vc-card-row1-left">
                              <span
                                className={`vc-card-network ${card.network === "Visa" ? "vc-card-visa" : "vc-card-mc"}`}
                              >
                                {card.network === "Visa" ? (
                                  "VISA"
                                ) : (
                                  <span className="vc-mc-card">
                                    <span />
                                    <span />
                                  </span>
                                )}
                              </span>
                              <span className="vc-card-label">
                                {card.label}
                              </span>
                            </div>
                            <span
                              className={`vc-card-badge ${getStatusColor(card.status)}`}
                            >
                              {card.status === "Active" ? "▼" : "●"}{" "}
                              {card.status}
                            </span>
                          ...
                          <div className="vc-card-row2">
                            ...
                            <div className="vc-card-flag-wrap">
                              <img
                                src={card.flag}
                                alt={card.currency}
                                className="vc-card-flag-img"
                              />
                              {!isLocked && (
                                <button
                                  className="vc-eye-card-btn"
                          ...
                          <div className="vc-card-row3">
                            <p className="vc-card-holder">
                              {isLocked ? "••••• •••••••" : card.holder}
                            </p>
                            <div className="vc-card-expiry-block">
                              <span className="vc-card-expiry-label">
                                Expiry
                              </span>
                              <strong className="vc-card-expiry-date">
                                {isLocked ? "••/••" : card.expiry}
                              </strong>
                            </div>
                          </div>
                        ...
                        <div className="vc-card-actions">
                          {card.status === "Active" ? (
                            ...
                            <div
                              className={`vc-on-review-badge ${card.status === "Pending" ? "badge-pending" : "badge-review"}`}
                            >
                              <span className="vc-on-review-icon">
                                {card.status === "Pending" ? "⏳" : "🔍"}
                              </span>
                              <span>
                                {card.status === "Pending"
          ...
          {fundModal.open && fundModal.card && (
            <div
              ...
              onClick={() => setFundModal({ open: false, card: null })}
            >
              <div className="vc-modal" onClick={(e) => e.stopPropagation()}>
                <h3 className="vc-modal-title">
                  <ArrowDownLeft size={18} /> Fund Card
                </h3>
                <p className="vc-modal-sub">
                  Add funds to your {fundModal.card.currency} Virtual Card
                  ending in {fundModal.card.last4}
                </p>
                <div className="vc-modal-field">
                  <label>
                    Amount ({getCurrencySymbol(fundModal.card.currency)})
                  </label>
                  <div className="vc-amount-input-wrap">
                    <span className="vc-amount-symbol">
                      {getCurrencySymbol(fundModal.card.currency)}
                    </span>
                    <input
                      type="number"
          ...
          {withdrawModal.open && withdrawModal.card && (
            <div
              className="modal-overlay"
              onClick={() => setWithdrawModal({ open: false, card: null })}
            >
              <div className="vc-modal" onClick={(e) => e.stopPropagation()}>
                <h3 className="vc-modal-title">
                ...
                <p className="vc-modal-sub">
                  Withdraw from your {withdrawModal.card.currency} Card ••••{" "}
                  {withdrawModal.card.last4}
                  <br />
                  <span className="vc-modal-balance">
                    Available: {getCurrencySymbol(withdrawModal.card.currency)}
                    {withdrawModal.card.balance.toLocaleString()}
                  </span>
                </p>
                <div className="vc-modal-field">
                  <label>
                    Amount ({getCurrencySymbol(withdrawModal.card.currency)})
                  </label>
                  <div className="vc-amount-input-wrap">
                    <span className="vc-amount-symbol">
                      {getCurrencySymbol(withdrawModal.card.currency)}
                    </span>
                    <input
                      type="number"
                      className="vc-amount-input"
          ...
          {selectedCardDetail && (
            ...
            >
              <div
                className="vc-modal vc-detail-modal"
              ...
              >
                ...
                <div
                  className={`vc-detail-card-preview ${getCardGradient(selectedCardDetail)}`}
                >
                  <div className="vc-card-top">
                    <span
                      className={`vc-card-network ${selectedCardDetail.network === "Visa" ? "vc-card-visa" : "vc-card-mc"}`}
                    >
                      {selectedCardDetail.network === "Visa" ? (
                        "VISA"
                      ) : (
                        <span className="vc-mc-card">
                          <span />
                    ...
                    <span
                      className={`vc-card-badge ${getStatusColor(selectedCardDetail.status)}`}
                    >
                      {selectedCardDetail.status}
                    </span>
                  </div>
                  <div className="vc-card-number">
                    **** **** **** {selectedCardDetail.last4}
                  </div>
                  <div className="vc-card-bottom">
                    <p className="vc-card-holder">
                      {selectedCardDetail.holder}
                    </p>
                    <p className="vc-card-expiry-label">
                      Expiry <strong>{selectedCardDetail.expiry}</strong>
                    </p>
                  </div>
                </div>
                <div className="vc-detail-rows">
                  <div className="vc-detail-row">
                    ...
                    <span>**** **** **** {selectedCardDetail.last4}</span>
                  </div>
                  <div className="vc-detail-row">
                    <span>Currency</span>
                    <span
                    ...
                    >
                      <Flag
                        src={selectedCardDetail.flag}
                        alt={selectedCardDetail.currency}
                      />
                  ...
                  <div className="vc-detail-row">
                    <span>Network</span>
                    <span>{selectedCardDetail.network}</span>
                  </div>
                  <div className="vc-detail-row">
                    <span>Balance</span>
                    <span>
                      {getCurrencySymbol(selectedCardDetail.currency)}
                      {selectedCardDetail.balance.toLocaleString()}
                    </span>
                  </div>
                  <div className="vc-detail-row">
                    <span>Expiry</span>
                    <span>{selectedCardDetail.expiry}</span>
                  </div>
                  <div className="vc-detail-row">
                    <span>Status</span>
                    <span
                      className={`vc-history-status ${getStatusColor(selectedCardDetail.status)}`}
                    >
                      {selectedCardDetail.status}
                    </span>
                  </div>
