...
const Wallet = () => {
  ...
  const [isHistoryFilterModalOpen, setIsHistoryFilterModalOpen] =
    useState(false);
  ...
  const fetchTransactionHistory = async () => {
    ...
    try {
      ...
      const transformedTransactions =
        transformApiResponseToTransactions(response);
  ...
  const transformApiResponseToTransactions = (apiData: any): Transaction[] => {
    ...
    const items: any[] = Array.isArray(apiData)
      ? apiData
      : (apiData?.data?.content ?? apiData?.content ?? []);
    ...
    return items.map((item: any) => {
      ...
      // Prefer new nested amount fields, fall back to flat legacy fields
      const net = Number(
        amountObj.net ?? item.netAmount ?? item.grossAmount ?? item.amount ?? 0,
      );
      const currency = amountObj.currency || item.currencyType || "NGN";
      const symbol = amountObj.symbol || item.symbol || "";
      ...
      return {
        ...
        transactionId: item.transactionId || item.id || undefined,
        sessionId: item.sessionId || undefined,
        referenceNo:
          item.reference || item.referenceId || item.referenceNo || undefined,
        terminalId: item.terminalId || undefined,
        erId: item.erId || undefined,
        accountHolder:
          item.user?.accountHolder || item.accountHolder || undefined,
        previousBalance:
          Number(balanceObj.previous ?? item.previousBalance ?? NaN) ||
          undefined,
        availableBalance:
          Number(balanceObj.available ?? item.availableBalance ?? NaN) ||
          undefined,
            ...
            <div className="wallet-header-wrapper">
                ...
                <div className="wallet-currency-selector">
                  ...
                  >
                    <span className="currency-option">
                      {selectedCurrency.code}
                      <i
                        className={`fa ${isDropdownOpen ? "fa-caret-up" : "fa-caret-down"} text-light`}
                        aria-hidden="true"
                        style={{
                          color: "#fff",
                          fontSize: "15px",
              ...
              <div className="balance-row">
                ...
                <div className="amount">
                  ...
                  {showBalance
                    ? formatBalance(currentBalance.toString())
                    : "*****"}
                </div>
              </div>
            </div>
                ...
                <div className="header-controls">
                  ...
                  <div className="filter-toggle-csontainer">
                    ...
                    >
                      ...
                      <i
                        className={`fa ${showDateFilter ? "fa-caret-up" : "fa-caret-down"}`}
                      />
                    </button>
                  </div>
                </div>
                  ...
                  <div className="active-filter-badge">
                    <div className="badge-content">
                      ...
                      <span className="badge-text">
                        {dateFilter.startDate || "Beginning"} →{" "}
              ...
              {transactionsLoading ? (
                ...
                <div className="transactions-list">
                  {currentTransactions.map((transaction) => {
                    ...
                    const isCredit = ["deposit", "credited", "swap"].includes(
                      transaction.type,
                    );
                    const isDebit = [
                      "withdrawal",
                        ...
                        <div className="transaction-summary">
                          ...
                          <div className="transaction-info">
                            <div className="transaction-main">
                              ...
                              >
                                {transaction.currency} {amountSign}
                          ...
                          <div
                            className="history-transaction-status"
                        ...
                        {isExpanded && (
                          <div className="transaction-details-panel">
                            <div className="details-grid">
                              ...
                              {/* Gross amount */}
                              {transaction.originalData?.amount?.gross !==
                                undefined && (
                                <div className="detail-item">
                                  <span className="detail-label">
                                    Gross Amount:
                                  ...
                                  <span className="detail-value">
                                    {transaction.originalData.amount.symbol ||
                              ...
                              {/* Fee */}
                              {transaction.originalData?.amount?.fee !==
                                undefined &&
                                Number(transaction.originalData.amount.fee) !==
                                  0 && (
                                  <div className="detail-item">
                              ...
                              {transaction.previousBalance !== undefined && (
                                <div className="detail-item">
                                  ...
                                  <span className="detail-value">
                                    {transaction.currency}{" "}
                                    {formatAmount(transaction.previousBalance)}
                                  </span>
                                </div>
                              )}
                              ...
                              {/* Running Balance (new nested field) */}
                              {transaction.originalData?.balance?.running !==
                                undefined && (
                                <div className="detail-item">
                                  <span className="detail-label">
                                    Running Balance:
        ...
        >
          ...
          >
            ...
            <div className="wallet-search-container">
              <span className="wallet-search-icon-inside">
                <Search size={16} />
              </span>
              <input
                type="text"
            ...
            <div
              className={`wallet-country-list ${isScrolling ? "is-scrolling" : ""}`}
            ...
            >
              {filteredCurrencies.map((c) => (
                ...
                >
                  ...
                  <div
                    className={`wallet-radio-outer ${selectedCurrency.code === c.code ? "checked" : ""}`}
                  >
              ...
              {filteredTabs.length > 0 ? (
                filteredTabs.map((tab) => (
                  ...
                  >
                    <span>{tab}</span>
                    <div
                      className={`radio-outer ${tab === activeTab ? "checked" : ""}`}
                    >
                      <div className="radio-inner"></div>
                    </div>
                  </div>
