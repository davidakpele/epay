...
export const setAuthToken = (
  payload: Partial<UserData> & { token?: string } = {},
) => {
  const {
    token = "",
    username = "",
  ...
  document.cookie = `data=${encodeURIComponent(secret_data)}; path=/; samesite=Lax`;
...
export const removeAuthToken = () => {
  ...
  document.cookie = `data=; path=/; samesite=Lax; expires=Thu, 01 Jan 1970 00:00:00 GMT`;
  document.cookie = `jwt=; path=/; samesite=Lax; expires=Thu, 01 Jan 1970 00:00:00 GMT`;
  window.location.replace("/");
};

// --- Getters ---

const getFromStorage = <K extends keyof UserData>(
  key: K,
): UserData[K] | null => {
  try {
...
const updateStorage = (updater: (user: UserData) => void) => {
  ...
  document.cookie = `data=${encodeURIComponent(updatedData)}; path=/; samesite=Lax`;
};

export const setActiveWallet = (currency: string) => {
  updateStorage((user) => {
    user.active_wallet = currency;
  });
};

export const setWalletContainer = (
  wallet: UserWallet[],
  isPinSet: boolean,
  id: string | number,
) => {
...
export const updateNotificationContainer = (
  notificationData: Omit<UserNotification, "date"> & { date?: string },
) => {
  updateStorage((user) => {
