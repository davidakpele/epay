        ...
        if (xhr.readyState === 4) {
          ...
          if (xhr.status >= 200 && xhr.status < 300) {
            ...
            try {
              ...
              errorPayload = { ...errorResponse, status: xhr.status };
            } catch (e) {
              errorPayload.message = xhr.responseText || `Error: ${xhr.status}`;
            }
            if (xhr.status === 401 && !isAuthPage) {
              window.location.href = "/auth/logout";
              return;
            }
