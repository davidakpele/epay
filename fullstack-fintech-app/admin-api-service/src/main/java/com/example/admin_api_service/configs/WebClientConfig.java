...
public class WebClientConfig {
    ...
    @Value("${virtual-card-service.base-url}")        
    private String virtualCardServiceBaseUrl;
    ...
    public WebClient walletServiceWebClient(WebClient.Builder webClientBuilder) {
        return webClientBuilder.baseUrl(walletServiceBaseUrl).build();
    }

    @Bean
    public WebClient historyServiceWebClient(WebClient.Builder webClientBuilder) {
        return webClientBuilder.baseUrl(historyServiceBaseUrl).build();
    }

    @Bean
    public WebClient authServiceWebClient(WebClient.Builder webClientBuilder) {
        return webClientBuilder.baseUrl(authServiceBaseUrl).build();
    }

    @Bean
    public WebClient bankListServiceWebClient(WebClient.Builder webClientBuilder) {
        return webClientBuilder.baseUrl(bankListServiceBaseUrl).build();
    }

    @Bean
    public WebClient notificationServiceWebClient(WebClient.Builder webClientBuilder) {
        return webClientBuilder.baseUrl(notificationServiceBaseUrl).build();
    }

    @Bean
    public WebClient blacklistServiceWebClient(WebClient.Builder webClientBuilder) {
        return webClientBuilder.baseUrl(blacklistServiceBaseUrl).build();
    }

    @Bean
    public WebClient revenueServiceWebClient(WebClient.Builder webClientBuilder) {
        return webClientBuilder.baseUrl(revenueServiceBaseUrl).build();
    }

    @Bean
    public WebClient escrowServiceWebClient(WebClient.Builder webClientBuilder) {
        return webClientBuilder.baseUrl(escrowServiceBaseUrl).build();
    }

    @Bean
    public WebClient virtualCardServiceWebClient(WebClient.Builder webClientBuilder) {  // ← NEW
        return webClientBuilder.baseUrl(virtualCardServiceBaseUrl).build();
    }
}